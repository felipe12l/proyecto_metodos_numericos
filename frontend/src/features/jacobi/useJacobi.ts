...
export function useJacobi() {
  ...
  const [errorPorcentaje, setErrorPorcentaje] = useState<string>('0.001');

  const [resultado, setResultado]       = useState<number[] | null>(null);
  const [iterations, setIteraciones]   = useState<number | null>(null);
  ...
  const calculate = async () => {
    ...
    try {
      ...
      const resp = await axios.post<JacobiResult>('/jacobi', payload);

      setResultado(resp.data.solution);
      setConverged(resp.data.converged);
      setIteraciones(resp.data.iterations);
    } catch (err: any) {
